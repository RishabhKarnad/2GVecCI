...
class biv_model:
    ...
    def build_effect_region(self):
        ...
        for i in range(self.samplesize):
            Y_vector[i][:] += np.matmul(self.interaction_matrix, self.cause.values[i][:])
...
def full_conditioning_ind_test(X, Y, alpha=0.01, type='both', CI_test_method='ParCorr', linear = True, fit_intercept = False, random_state = None):
    ...
    ## INPUT PARAMETERS:
    ## X,Y = pp.Dataframes containing regional variables,
    ...
    if CI_test_method == 'RFDC':
        ...
        if type == 'both':
            ...
            for var1 in range(len(Y.var_names)):
                for var2 in range(var1 + 1, len(Y.var_names)):
                    ...
                    valY, pvalY = RFDC(np.array(Y.values[:, var1:var1 + 1]), np.array(Y.values[:, var2:var2 + 1]),
                                                       Z=removedY, random_state=random_state)
                    if pvalY < alpha:
                        edgecounterY += 1
                    cond_setY = np.concatenate((removedY, X.values), axis=1)
                    valResY, pvalResY = RFDC(np.array(Y.values[:, var1:var1 + 1]), np.array(Y.values[:, var2:var2 + 1]),
            ...
            for var1 in range(len(X.var_names)):
                for var2 in range(var1 + 1, len(X.var_names)):
                    ...
                    valX, pvalX = RFDC(np.array(X.values[:, var1:var1 + 1]), np.array(X.values[:, var2:var2 + 1]),
                                                       Z=np.array(removedX),random_state=random_state)
                    if pvalX < alpha:
                        edgecounterX += 1
                    cond_setX = np.concatenate((removedX, Y.values), axis=1)
                    valResX, pvalResX = RFDC(np.array(X.values[:, var1:var1 + 1]), np.array(X.values[:, var2:var2 + 1]),
        ...
        if linear == True:
            ...
            if type == 'both':
                residualsY = Regression['X_to_Y'].residuals.values
                edgecounterY = 0
                edgecounterResY = 0
                max_edgenumberY = len(Y.var_names) * (len(Y.var_names) - 1) / 2
                max_edgenumberX = len(X.var_names) * (len(X.var_names) - 1) / 2
                ...
                dict['edge density of Y'] = edgecounterY / max_edgenumberY
                dict['edge density of Res Y'] = edgecounterResY / max_edgenumberY
                residualsX = Regression['Y_to_X'].residuals.values
                edgecounterX = 0
                edgecounterResX = 0
                for var1 in range(len(X.var_names)):
                    for var2 in range(var1 + 1, len(X.var_names)):
            ...
            if type == 'minimal':
                ...
                if len(Y.var_names) > len(X.var_names) or random_choice == 1:
                    ...
                    for var1 in range(len(Y.var_names)):
                        for var2 in range(var1 + 1, len(Y.var_names)):
                            removedY = np.delete(Y.values, (var1, var2), 1)
                            valY, pvalY = CI_test.run_test_raw(Y.values[:, var1:var1 + 1], Y.values[:, var2:var2 + 1],
                                                               z=removedY)
                            if pvalY < alpha:
                                edgecounterY += 1
                            removedResY = np.delete(residualsY, (var1, var2), 1)
                ...
                for var1 in range(len(Y.var_names)):
                    for var2 in range(var1 + 1, len(Y.var_names)):
                        ...
                        if pvalY < alpha:
                            edgecounterY += 1
                        cond_setY = np.concatenate((removedY,X.values),axis=1)
                        valResY, pvalResY = CI_test.run_test_raw(Y.values[:, var1:var1 + 1], Y.values[:, var2:var2 + 1],
                                                                 z=cond_setY)
                        if pvalResY < alpha:
                            edgecounterResY += 1
                dict['number of nodes Y'] = len(Y.var_names)
                ...
                for var1 in range(len(X.var_names)):
                    for var2 in range(var1 + 1, len(X.var_names)):
                        ...
                        if pvalX < alpha:
                            edgecounterX += 1
                        cond_setX = np.concatenate((removedX, Y.values), axis=1)
                        valResX, pvalResX = CI_test.run_test_raw(X.values[:, var1:var1 + 1], X.values[:, var2:var2 + 1],
                                                                 z=cond_setX)
                        if pvalResX < alpha:
                            edgecounterResX += 1
                dict['number of nodes X'] = len(X.var_names)
            ...
            if type == 'minimal':
                ...
                if len(Y.var_names) > len(X.var_names) or random_choice == 1:
                    ...
                    for var1 in range(len(X.var_names)):
                        for var2 in range(var1 + 1, len(X.var_names)):
                            removedX = np.delete(X.values, (var1, var2), 1)
                            valX, pvalX = CI_test.run_test_raw(X.values[:, var1:var1 + 1], X.values[:, var2:var2 + 1],
                                                               z=removedX)
                            if pvalX < alpha:
                                edgecounterX += 1
                            cond_setX = np.concatenate((removedX, Y.values), axis=1)
...
def identify_causal_direction(X,Y,alpha=0.01, type = 'both',CI_test_method='ParCorr',ambiguity = None, test = 'full', max_sep_set = None, linear = True, fit_intercept = False, random_state = None):
    ...
    if type == 'both':
        ...
        if Y_criterion - X_criterion > ambiguity:
            comparison = 'X is the cause of Y'
        if X_criterion - Y_criterion  > ambiguity:
